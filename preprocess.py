...
def save_our_data_as_numpy_array(max_len,max_len2,origin_path,destination_path):
    labels = get_labels(origin_path)
    for label in labels:
        if os.path.exists(destination_path + label + '.npy'):
            print(label, 'exists...skipping')
        ...
        # Init mfcc vectors
            ...
            wavfiles = []
#            for file in os.listdir(origin_path + '/' + label):
            for wavfile in os.listdir(origin_path + '/' + label + '/'):
                current_path = origin_path + '/' + label + '/' + wavfile
                if wavfile[-3:] != 'txt':
                    wavfiles.extend([current_path])
            for wavfile in tqdm(wavfiles, "Saving vectors of label - '{}'".format(label)):
                splits = split_audio_file(file_path=wavfile)
